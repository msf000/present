...
import { Users, CheckCircle, XCircle, Clock, BrainCircuit, FileDown, AlertTriangle, Star, PieChart as PieChartIcon, FileText, Activity, Filter, ArrowRight, ClipboardCheck } from 'lucide-react';
...
const Dashboard: React.FC<DashboardProps> = ({ students, records, settings, onNavigate }) => {
  ...
  const { riskStudents, topStudents, recentExceptions } = useMemo(() => {
    ...
    return { 
        ...
        topStudents: top.sort((a, b) => b.rate - a.rate).slice(0, 5),
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
         ...
         <div className="bg-white p-4 rounded-xl shadow-sm border border-slate-200">
            <h3 className="font-bold text-slate-800 mb-4 flex items-center gap-2">
               ...
               الطلاب الأكثر غياباً (أقل من {settings.attendanceThreshold}%)
            </h3>
            <div className="space-y-3">
              {riskStudents.length > 0 ? (
                riskStudents.map(({ student, rate }) => (
                  ...
                  >
                     <div className="flex items-center gap-3">
                       <div className="w-8 h-8 rounded-full bg-white text-red-500 flex items-center justify-center font-bold text-xs border border-red-100 shadow-sm">
                         {rate}%
                       </div>
                       <div>
                ...
                <p className="text-center text-slate-400 py-8 flex flex-col items-center">
                    ...
                    لا يوجد طلاب في دائرة الخطر. ممتاز!
                </p>
              )}
            </div>
         </div>
